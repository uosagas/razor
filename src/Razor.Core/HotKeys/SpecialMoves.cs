...
// Portiert aus Razor CE (Razor/HotKeys/SpecialMoves.cs) — Phase 3c2.
// Vollstaendig: War/Peace-Toggles (0x72), AOS-Abilities (0xD7 + 0xBF/0x21-
// Anzeige-Reset im Client) inkl. der kompletten Waffen-Tabelle, Pre-AOS
// Stun/Disarm (0xBF sub 0x0A/0x09). AOSAbility liegt bereits in Core/Enums.cs.
...
namespace Assistant
{
    public class SpecialMoves
    {
        ...
        {
            ClientProxy.SendToServer(new SetWarMode(!World.Player.Warmode));
        }

        private static void ToggleWar()
        {
            ClientProxy.SendToClient(new SetWarMode(true));
            ClientProxy.SendToServer(new SetWarMode(true));
        }

        private static void TogglePeace()
        {
            ClientProxy.SendToClient(new SetWarMode(false));
            ClientProxy.SendToServer(new SetWarMode(false));
        }
        ...
        public static void SetPrimaryAbility()
        {
            ...
            if (a != AOSAbility.Invalid)
            {
                ClientProxy.SendToServer(new UseAbility(a));
