...
// Portiert aus Razor CE (Razor/Network/PacketHandler.cs).
// Abweichungen:
//  * Register*/Remove* sind public statt internal (Plugin-/Test-Zugriff).
//  * Exception-Handling: Console statt Engine.LogCrash + MessageDialog (WinForms).
        ...
        private static bool ProcessViewers(List<PacketViewerCallback> list, PacketReader p)
        {
            ...
            if (list != null)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    {
                        // TODO Razor CE: Engine.LogCrash(e) + MessageDialog (WinForms).
                        Console.WriteLine($"[Razor] Packet viewer exception (0x{p.PacketID:X2}): {e}");
                    }
                }
            }
        ...
        private static bool ProcessFilters(List<PacketFilterCallback> list, Packet p)
        {
            ...
            if (list != null)
            {
                for (int i = 0; i < list.Count; i++)
                {
                    ...
                    {
                        // TODO Razor CE: Engine.LogCrash(e) + MessageDialog (WinForms).
                        Console.WriteLine($"[Razor] Packet filter exception (0x{p.PacketID:X2}): {e}");
                    }
                }
            }
