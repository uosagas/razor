...
// Portiert aus Razor CE (Razor/Scripts/Engine/Interpreter.cs) fuer den
// UOSagas-Razor-Port. Reiner C#-AST-Executor + statische Registry/Scheduler.
// Einzige externe Abhaengigkeit: Serial (Assistant.Serial, bereits vorhanden).
// 1:1 uebernommen, damit CE-.razor-Scripts identisch laufen.
...
namespace Assistant.Scripts.Engine
{
    ...
    public class Script
    {
        ...
        private bool CompareOperands(ASTNodeType op, IComparable lhs, IComparable rhs)
        {
            ...
            {
                // Outlands-Erweiterung: string-Ergebnisse (z.B. atlist), die als
                // Serial/Zahl parsebar sind, fuer 'as' zulassen. CE-Verhalten
                // fuer uint-lhs unveraendert.
                if (lhs is string s)
                {
                    if (s.StartsWith("0x") &&
                        uint.TryParse(s.Substring(2), System.Globalization.NumberStyles.HexNumber, Interpreter.Culture, out uint hex))
                        lhs = hex;
                    else if (uint.TryParse(s, out uint dec))
                        lhs = dec;
                }
    ...
    public static class Interpreter
    {
        ...
        // --- Registry-Introspektion (Ergaenzung ggü. Razor CE) -------------------
        // Quelle der Wahrheit fuer: Doku-Export (tools/LanguageExport),
        // Editor-Autocomplete und den Sprachoberflaechen-Test. Damit kann die
        // Doku nicht von der tatsaechlich registrierten Sprache abweichen.

        /// <summary>Alle registrierten Command-Namen.</summary>
        public static IEnumerable<string> RegisteredCommands => _commandHandlers.Keys;

        /// <summary>Alle registrierten Expression-Namen.</summary>
        public static IEnumerable<string> RegisteredExpressions => _exprHandlers.Keys;

        /// <summary>Alle registrierten Alias-Namen.</summary>
        public static IEnumerable<string> RegisteredAliases => _aliasHandlers.Keys;
