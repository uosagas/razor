...
// Portiert aus Razor CE (Razor/Core/Item.cs). Das Layer-Enum lebt bereits in
// Core/Enums.cs (Phase 2a) und wird hier nicht erneut definiert.
// Phase 2d NACHGERUESTET (werktreu zu Razor CE):
//  * Agent.InvokeItemCreated im Konstruktor,
//  * Counter.Count/Uncount + SearchExemptionAgent.IsExempt in Container/
//    UpdateContainer/Remove.
// Feature-Paritaet (2026-07-17): AutoStack (AutoStackResource) + AutoSearch
// (Container automatisch oeffnen, IgnoreGumps) in UpdateContainer — CE 1:1.
// STUBS/ENTFERNT gegenueber Razor CE (nur Weltzustand, keine Seiteneffekte):
//  * Layer-Getter ohne Tiledata-Fallback (TODO: Tiledata via IClientServices)
//  * FindItemByName/DisplayName (brauchen Tiledata) entfallen (TODO)
//  * RemoveRequest ohne 0.25s-Timer: entfernt sofort (TODO: Timer-Port)
//  * IsTwoHanded ohne Overrides-Datei
//  * HousePacket/MakeHousePacket (Ultima-MUL-Zugriff) entfallen
...
namespace Assistant
{
    public partial class Item : UOEntity
    {
        ...
        public Item(Serial serial) : base(serial)
        {
            ...
            Agents.Agent.InvokeItemCreated(this);
        }
        ...
        public string Name
        {
            get
            {
                ...
                // Kein OPL-/SingleClick-Name: Tiledata-Name (DataService).
                string tdName = Assistant.ItemData.GetName(_itemId.Value);
        ...
        public Layer Layer
        {
            get
            {
                // Razor CE: fuer Wearable/Armor/Weapon ohne gueltigen Layer auf den
                // Tiledata-Layer (ItemData.Quality) zurueckfallen — via DataService.
                if (_layer < Layer.FirstValid || _layer > Layer.LastValid)
                {
                    TileItemData td = Assistant.ItemData.Get(_itemId.Value);
                    if (td.Valid && td.IsEquippable)
                        _layer = td.Layer;
                }
        ...
        /// <summary>
        /// Razor CE: Item.FindItemsByName — Container-Inhalt per Tiledata-Name
        /// (exakt, case-insensitive), optional rekursiv.
        /// </summary>
        public List<Item> FindItemsByName(string name, bool recurse)
        {
            ...
            for (int i = 0; i < _items.Count; i++)
            {
                ...
                if (string.Equals(Assistant.ItemData.GetName(item.ItemID.Value), name, StringComparison.OrdinalIgnoreCase))
                {
                    items.Add(item);
                }
                ...
                if (recurse)
                {
                    List<Item> recurseItems = item.FindItemsByName(name, true);
                    if (recurseItems.Count > 0)
                    {
                        items.AddRange(recurseItems);
                    }
        ...
        public bool UpdateContainer()
        {
            ...
            if (World.Player != null && (IsChildOf(World.Player.Backpack) || IsChildOf(World.Player.Quiver)))
            {
                bool exempt = Agents.SearchExemptionAgent.IsExempt(this);
                if (!exempt)
                    Counter.Count(this);
                ...
                if (_isNew)
                {
                        ...
                        Config.GetBool("AutoSearch"))
                    {
                        ...
                        for (int c = 0; c < Contains.Count; c++)
                        {
                            ...
                            if (icheck.IsContainer && !Agents.SearchExemptionAgent.IsExempt(icheck) &&
        ...
        /// <summary>Leert den AutoStack-Dedup-Cache (bei World.Clear — CE leert ihn
        /// nie, riskiert damit aber Serial-Kollisionen ueber Sessions hinweg).</summary>
        public static void ClearAutoStackCache()
        {
            m_AutoStackCache.Clear();
        }

        // Razor CE 1:1: frisch aufgesammelte Ressourcen (Erz/Fisch/Holz/Granit/Sand)
        // automatisch auf einen gleichen Bodenstapel neben dem Spieler (Range 2)
        // bzw. auf die Spielerposition droppen.
        ...
        {
            // Razor CE: Entfernen um 0.25s verzoegern, damit ein direkt folgendes
            // Update (haeufig beim Bewegen: Remove + sofortiges Re-Add) das Item
            // ueber CancelRemove "retten" kann. Ohne diese Verzoegerung verschwinden
            // getragene/Container-Items faelschlich (out of range).
        ...
        public bool IsTwoHanded
        {
            get
            {
                ...
                // TODO Razor CE fragt zusaetzlich Overrides.TwoHanded (Konfig-Datei) ab.
