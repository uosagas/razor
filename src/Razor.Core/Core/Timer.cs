...
// Portiert aus Razor CE (Razor/Core/Timer.cs): MinHeap + Timer.
// Abweichung: Razor CE pumpt Slice() ueber eine Win32-Message (PostMessage an
// das Hauptfenster, ChangedNextTick startete dort einen System-Timer).
// Im UOSagas-Port ruft RazorPlugin.OnTick (Game-Thread, 1x pro Frame)
// Timer.Slice() auf — ChangedNextTick reduziert sich damit auf den
// "allowImmediate"-Sofortlauf. Zeitbasis unveraendert DateTime.UtcNow.
...
namespace Assistant
{
    ...
    public abstract class Timer : IComparable
    {
        ...
        {
            // Razor CE stellte hier den Win32-Timer auf das naechste Faelligkeits-
            // intervall. Der UOSagas-Port wird pro Frame gesliced; nur der
            // Sofortlauf (Timer mit Delay <= 0 direkt beim Start) bleibt erhalten.
            if (!m_Heap.IsEmpty)
            {
        ...
        /// <summary>
        /// Verarbeitet alle faelligen Timer. Wird von RazorPlugin.OnTick
        /// (1x pro Frame auf dem Game-Thread) aufgerufen.
        /// </summary>
