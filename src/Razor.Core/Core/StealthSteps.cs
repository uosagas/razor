...
// Portiert aus Razor CE (Razor/Core/StealthSteps.cs) — 1:1.
// Hide() beim Unsichtbar-Werden (MobileUpdate/MobileIncoming des Spielers),
// OnMove() pro Schritt (RazorPlugin.OnPlayerPositionChanged), Unhide() beim
// Sichtbar-Werden. Nach 30 Schritten gilt Stealth als verbraucht (CE).
    ...
    public class StealthSteps
    {
        ...
        public static void OnMove()
        {
            if (m_Hidden && Config.GetBool("CountStealthSteps") && World.Player != null)
            {
                ...
                {
                    World.Player.OverheadMessage(Config.GetInt("SysColor"), message);
                }
                else
                {
